...
### 1. 下载代码： 
      ...
      yum install -y wget && wget https://github.com/tenondvpn/tenonvpn-join/archive/5.0.3.zip
   

### 2. 初始化环境，执行完成后会重启系统，程序也会自动启动

      yum install -y unzip && unzip 5.0.3.zip && mv tenonvpn-join-5.0.3 tnet && cd tnet && sh init_env.sh
...
### 2. 初始化环境，执行完成后会重启系统，程序也会自动启动
      ...
      yum install -y unzip && unzip 5.0.3.zip && mv tenonvpn-join-5.0.3 tnet && cd tnet && sh init_env.sh min_port max_port
